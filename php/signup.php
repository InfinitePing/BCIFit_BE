...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(empty($name)){
    ...
    }else if(!empty($name) && !empty($email) && !empty($pass) && !empty($pass_conf) && filter_var($email, FILTER_VALIDATE_EMAIL)){
        ...
        try{
            ...
            //sql querry
            $signup_sql = "INSERT INTO users (name, email, pass_hash) VALUES (?, ?, ?)";
        ...
        try{
            ...
            $stmt->bind_param("sss", $esc_name, $esc_email, $esc_pass);
        }catch(Exception $e) {
            die(messageH1($e->getMessage()));
        };
        ...
        try{
            $stmt->execute();
            echo(messageH1s("Register Success! Please Log In"));
            $stmt->close();
            exit;
        }catch(Exception $e) {
